...
/// 游戏事件数据类。
/// </summary>
internal class AEventDelegateData
{
    private readonly AEventType _aEventType = 0;
    ...
    internal AEventDelegateData(AEventType aEventType)
    {
        _aEventType = aEventType;
    }
    ...
    internal bool AddHandler(Delegate handler)
    {
        ...
        {
            ADebug.LogWarning("Repeated Add Handler");
            return false;
        }
    ...
    internal void RmvHandler(Delegate handler)
    {
        ...
        else
        {
            ...
            {
                ADebug.LogWarning($"Delete handle failed, not exist, EventId: {_aEventType.ToString()}");
            }
        }
    }
