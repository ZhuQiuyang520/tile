...
namespace excel2json
{
    ...
    class JsonExporter
    {
        ...
        public JsonExporter(ExcelLoader excel, bool lowcase, bool exportArray, string dateFormat, bool forceSheetName, int headerRows, string excludePrefix, bool cellJson, bool allString, string[] dictSheetName)
        {
            ...
            { // mutiple sheet
                ...
                //{
                //    List<Dictionary<string, object>> sheetValue = convertSheetToArrayWithoutObj(sheet, lowcase, excludePrefix, cellJson, allString);
                ...
                {
                    object sheetValue;
                    if (dictSheetName != null && Array.Exists(dictSheetName, element => element == sheet.TableName))
                    {
                        sheetValue = convertSheet(sheet, false, lowcase, excludePrefix, cellJson, allString);
                    }
                    else
                    {
                        sheetValue = convertSheetToArrayWithoutObj(sheet, lowcase, excludePrefix, cellJson, allString);
                    }

                    data.Add(sheet.TableName.Replace("<T>", ""), sheetValue);
                }
        ...
        private List<Dictionary<string, object>> convertSheetToArrayWithoutObj(DataTable sheet, bool lowcase, string excludePrefix, bool cellJson, bool allString)
        {
