...
{
    public class ResourceCtrl : ResourceDB, IRule
    {
        ...
        public bool AddItemValue(Item item, int _value, bool checkMax = false)
        {
            ...
            {
                // 如果是增加无限体力，走体力管理的接口
                ArcticRule.Instance.BurEmergencyQuit(_value);
                addSuccess = true;
            }
            else
            {
            ...
            if (addSuccess)
            {
                DeviateCenterChurn.PenMonopoly().Jump(CLagoon.Ox_OozeDampen_ + item.id, new DeviateHave(_value));
            }
            return addSuccess;
        }
        ...
        public bool SetItemValue(Item item, int newValue, bool checkValue = false)
        {
            ...
            if (success)
            {
                DeviateCenterChurn.PenMonopoly().Jump(CLagoon.Ox_OozeDampen_ + item.id, new DeviateHave(newValue - oldValue));
            }
            return success;
        }
        ...
        {
            if (string.IsNullOrEmpty(itemgroup_id) || !OozeWearyRule.Instance.NestHollow.ContainsKey(itemgroup_id))
            {
                return null;
            }
            else
            {
                return OozeWearyRule.Instance.NestHollow[itemgroup_id];
            }
        }
        ...
        public List<ItemGroup> GetItemGroupByIds(string shop_id, string itemgroup_id, string item_id, int item_num)
        {
            ...
            {
                Shop shop = PageRule.Instance.PenPageOnOf(shop_id);
