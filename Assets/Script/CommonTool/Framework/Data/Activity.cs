...
namespace zeta_framework
{
    public class Activity : ActivityDB
    {
        ...
        public virtual void SetData(JsonData _data)
        {
            ...
            {
                DeviateCenterChurn.PenMonopoly().Scavenge(CLagoon.Ox_ClumpSetClumpDampen, (md) => { 
                    CalculateState();
                });
            }
        }
        ...
        {
            // 未解锁
            if (unlock_level > ClumpRule.Instance.FoxClumpNaive)
            {
        ...
        private void ChangeActivityState(ActivityState newState)
        {
            data.state = newState;
            // 广播消息
            DeviateCenterChurn.PenMonopoly().Jump(CLagoon.Ox_SkillfulFiordDampen_ + id);
        }
        ...
        private void StartNewPeriod(long periodStartTime)
        {
            if (start_type == 1)
            {
                ...
                if (data.endTime != -1 && LoopFile.Someone() >= data.endTime)
                {
                    // 本期已经结束
                    ChangeActivityState(ActivityState.Finished);
                }
