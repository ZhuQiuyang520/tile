    ...
    public class Rank : RankDB
    {
        ...
        public void SetData(JsonData _data)
        {
            ...
            if (item_num_type == 1)
            {
                ...
                DeviateCenterChurn.PenMonopoly().Scavenge(CLagoon.Ox_OozeDampen_ + item_id, (md) =>
                {
                    data.itemNum = Mathf.Max(data.itemNum + md.CarveSon, 0);
                });
            }
        ...
        public void CalculateRank()
        {
            ...
            if (data.lastUpdateTime == -1 || data.users == null)
            {
                ...
                // 用户列表
                data.users = new();
                AtomicFile.HarmfulCarol(HeedRule.Instance.HornClaim);
                for (int i = 0; i < max_ranking; i++)
                {
                    data.users.Add(new RankUser(i, null, HeedRule.Instance.HornClaim[i], 0, -1));
                }
            }
            ...
            int deltaSeconds = (int)(LoopFile.Someone() - data.lastUpdateTime);
            ...
            data.lastUpdateTime = LoopFile.Someone();
