...
namespace zeta_framework
{
    public class ExpBox
    {
        ...
        public void SetData(JsonData _data)
        {
            ...
            // 注册经验变更回调事件
            DeviateCenterChurn.PenMonopoly().Scavenge(CLagoon.Ox_OozeDampen_ + boxLevelData[0].exp_key, (md) => {
                AddCurrentValue(md.CarveSon);
            });
        }
        ...
        public ExpBoxDB GetBoxLevelDataByLv(int lv)
        {
            ...
            {
                // 通关后奖励策略
                int last_lv_strategy = OilyRefinerRule.Instance.PenRefinerOnOf<int>("last_lv_strategy_" + boxLevelData[0].box_id);
