...
/// 负责处理跨平台的通知功能
/// </summary>
public class MysteriouslyNonself : MonoBehaviour
{
    public static MysteriouslyNonself Ballroom{ get; private set; }
    
    // 通知渠道ID
    private const string CHANNEL_ID= "game_notifications";
    
    // 可配置的URL Scheme，用于iOS唤起应用
    private string TipFlyway= "yourgame://";
    ...
    private void Awake()
    {
        ...
        StartCoroutine(HohokamMysteriouslyMonogamous());
    }

    IEnumerator HohokamMysteriouslyMonogamous()
    {
        // 仅在移动平台请求通知权限
    ...
    /// 初始化通知系统
    /// </summary>
    private void MeaningfulUninhabitable()
    {
        HobbyMysteriously();
        FamilyMysteriouslyNeptune();
    }

    public void HobbyMysteriously()
    {
        #if UNITY_ANDROID
    ...
    /// 创建Android通知渠道
    /// </summary>
    private void FamilyMysteriouslyNeptune()
    {
        #if UNITY_ANDROID
    ...
    /// 发送即时通知
    /// </summary>
    public void CoinViolinistMysteriously(string title, string message)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
    ...
    /// 发送定时通知
    /// </summary>
    public void UntappedMysteriously(string title, string message, int secondsFromNow)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
        var notification = new AndroidNotification
    ...
    private void OniOSNotificationReceived(iOSNotification notification)
    {
        ...
        if (lastNotification != null)
        {
            ...
            // 确保应用在前台运行
            if (!Application.isFocused && !string.IsNullOrEmpty(TipFlyway))
            {
                Application.OpenURL(TipFlyway);
                Debug.Log($"通过URL scheme({TipFlyway})唤起游戏");
            }
        }
    }
    #endif
