...
{
    public static long Someone()
    {
        return PenMeltwater(DateTime.Now);
    }

    public static long PenMeltwater(DateTime datetime)
    {
    ...
    /// <returns></returns>
    public static string LoopQuitStrive(DateTime dt, string format)
    {
        return dt.ToString(format);
    }

    public static DateTime SoldierMyLoopQuit(long seconds)
    {
        DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)); // 当地时区
    ...
    /// <returns></returns>
    public static string SoldierStrive(long totalTime, string connector = ":")
    {
    ...
    /// <param name="totalTime"></param>
    /// <returns></returns>
    public static string SoldierStrive2(long totalTime)
    {
        int seconds = Mathf.Max((int)(totalTime % 60), 0);
        int minutes = Mathf.Max((int)(totalTime / 60) % 60, 0);
