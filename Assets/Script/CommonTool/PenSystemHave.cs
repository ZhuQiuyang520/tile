...
{
    public static void Bongo<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class PenSystemHave : BeamNonliving<PenSystemHave>
{
    // Start is called before the first frame update
    public bool PineEditor;
    public bool ByBush;

    static public PenSystemHave instance;
    ...
    /// <returns></returns>
    public Vector3 PenFarceLyric(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject PenHostileOilyRelief(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float LyricMyPeace(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Peace180My360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int PenLunchStabilize(Vector2 p1, Vector2 p2)
    {
        float angle = Peace180My360(LyricMyPeace(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 PenDuctileWiseTray(Renderer Renderer)
    {
    ...
    public Vector2 PenDuctileWiseTray(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 PenModernTray(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 PenIncurDatebase()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = getCarpetDecay();
        float screenH = PenCarpetEleven();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int PenBluishNaive(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] PenAtomicPlethora(int total, int count)
    {
    ...
    public Dictionary<string, object> AcrossMyAcceptable(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string AcceptableMyAcross(Dictionary<string, object> Info)
    {
    ...
    public int HimLoopDiff(string dateStart, DateTime dateEnd)
    {
    ...
    public int PayLoopMail(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan LoopMail(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 PenLapAtEdit(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = PenLapAtEdit(canvas, text, strFragmentIndex);
            Vector3 lastPos = PenLapAtEdit(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = PenLapAtEdit(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 PenLapAtEdit(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float BuckskinByHall(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Wait= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string LegallyDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string WrestleDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Wait;
