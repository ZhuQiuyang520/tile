...
public class ConestogaAssumption : MonoBehaviour
{
    void Start()
    {
    ...
    public static void YewBlue(GameObject PopBarUp,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void YewUtah(GameObject PopBarDisapper,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void DampenFloral(int startNum, int endNum,float delay, Text text,System.Action finish)
    {
    ...
    public static void DampenFloral(double startNum, double endNum, float delay, Text text, System.Action finish)
    {
        DampenFloral(startNum, endNum, delay, text, "", finish);
    }
    public static void DampenFloral(double startNum, double endNum, float delay, Text text, string prefix, System.Action finish)
    {
        DOTween.To(() => startNum, x => text.text = prefix + FloralFile.ZigzagMyBuy(x), endNum, 0.5f).SetDelay(delay).OnComplete(() =>
        {
            finish();
        });
    }
    ...
    public static void IsleAsiaThaw(GameObject GoldImage, int a, Vector2 StartPosition, Vector2 EndPosition, System.Action finish)
    {
        //如果没有就算了
        if (a == 0)
        {
    ...
    public static void IsleAsiaThaw(GameObject GoldImage, int a, Transform StartTF, Transform EndTF, System.Action finish) {
        IsleAsiaThaw(GoldImage, a, StartTF.position, EndTF.position, finish); 
    }
    ...
    /// <param name="Finish"></param>
    public static void DisabilityThresh(GameObject obj, float addPosition, System.Action Finish)
    {
