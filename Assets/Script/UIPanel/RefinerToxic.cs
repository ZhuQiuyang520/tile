...
public class RefinerToxic : FormUIBasin
{
...
[UnityEngine.Serialization.FormerlySerializedAs("Exit")]    public Button Yarn;
...
[UnityEngine.Serialization.FormerlySerializedAs("TryBtn")]
    ...
    private void DampenLopShade()
    {
        ...
        {
            SlayNeverSpiral.PenMonopoly().JumpNever("1025", OilyMimetic.PenMonopoly().PenAdmissionClump().ToString());
            SlayNeverSpiral.PenMonopoly().JumpNever("1033", OilyMimetic.PenMonopoly().PenAdmissionClump().ToString(), OilyMimetic.PenMonopoly().Clothe.ToString());
            ...
            SlayNeverSpiral.PenMonopoly().JumpNever("1029", OilyMimetic.PenMonopoly().PenAdmissionClump().ToString(), str.ToString());
            SlayNeverSpiral.PenMonopoly().JumpNever("1038", OilyMimetic.PenMonopoly().PenAdmissionClump().ToString(), str.ToString(), OilyMimetic.PenMonopoly().Clothe.ToString());
            UIMimetic.PenMonopoly().FlakeIceUI();
            UIMimetic.PenMonopoly().BlueUIBasin(nameof(OilyToxic));
            OilyMimetic.PenMonopoly().CrampPerceptual(StartChallengeState.SettingTryAgain);
        }
        else
        {
            HatchUIWork(GetType().Name);
