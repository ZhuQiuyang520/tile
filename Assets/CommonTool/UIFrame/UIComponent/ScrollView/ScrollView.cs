...
{
    //预支单体
    public ScrollViewItem itemCell;
    //scrollview
    ...
    //可见的itemList
    public List<ScrollViewItem> visibleList;
    //总共的dataList
    ...
    //更新item
    public void UpdateItem(int index, ScrollViewItem obj)
    {
        int d = allList[index];
    ...
    //item进入itemlist
    public void PushItem(ScrollViewItem obj)
    {
        itemList.Add(obj);
    ...
    void Scroll()
    {
        ...
        if (vy > rollUpTop && lastIndex < DataCount)
        {
            ...
            {
                ScrollViewItem obj = visibleList[0];
        ...
        if (vy < rollUpBottom - totalHeight && startIndex > 0)
        {
            ...
            if (visibleList.Count > 0)
            {
                ScrollViewItem obj = visibleList[visibleList.Count - 1];
