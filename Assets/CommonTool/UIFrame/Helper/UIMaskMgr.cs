...
public class UIMaskMgr : MonoBehaviour
{
    ...
    {
        if (UIManager.GetInstance().WaitUIForms.Count > 0 || UIManager.GetInstance().GetCurrentFormStack().Count > 0)
        {
            return;
        }
    ...
    public void CancelMaskWindow()
    {
        ...
        var openingPanels = UIManager.GetInstance().GetOpeningPanels(true);
        foreach (var panel in openingPanels)
        {
            var baseUIForm = panel.GetComponent<BaseUIForms>();
            if (baseUIForm != null && baseUIForm.CurrentUIType.UIForms_Type == UIFormType.PopUp)
            {
                hasOtherPopUp = true;
